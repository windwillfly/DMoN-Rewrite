...
class SalsaConverter(BaseConverter):
    ...
    def convert(self, edge_distance_threshold=0.4, frustum_length: float = 1.0, frustum_angle=math.pi / 3, ):
        ...
        for cur_ts in ff_timestamps:
            graph = self.graph_generator(cur_ts, edge_distance_threshold, fformation, frustum_angle, frustum_length,
                                         frustum_max_area, geometry)

            graphs.append(graph)

        return graphs

    def graph_generator(self, cur_ts, edge_distance_threshold, fformation, frustum_angle, frustum_length,
                        frustum_max_area, geometry, use_body_orientation=True):
        ...
        if self.show_ground_truth:
            ...
            for person_no in range(1, len(geometry) + 1):
                for other_person_no in range(person_no + 1, len(geometry) + 1):
                    ...
                    f1 = calc_frustum(person_feat, frustum_length=frustum_length, frustum_angle=frustum_angle, use_body=use_body_orientation)
                    f2 = calc_frustum(other_person_feat, frustum_length=frustum_length, frustum_angle=frustum_angle, use_body=use_body_orientation)
