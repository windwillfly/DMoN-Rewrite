...
class DMoN(tf.keras.layers.Layer):
    ...
    def call(
        ...
        assignments, collapse_loss, features_pooled, spectral_loss = self.create_network(adjacency, features)
    ...
    def create_network(self, adjacency, features):
        ...
        normalizer = tf.math.divide_no_nan(tf.matmul(normalizer_left,
                                                     normalizer_right) / 2, number_of_edges)
        spectral_loss = tf.math.divide_no_nan(-tf.linalg.trace(graph_pooled -
        ...
        return assignments, collapse_loss, features_pooled, spectral_loss

    def assert_input_valid(self, adjacency, features):
